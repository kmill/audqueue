...
function Database(loaded_callback) {
  this.songs = [];

  var self = this;
  get_songs(function (songs) {
    _.each(songs, function (song) {
      self.add_song(new Song(song));
    });
    if (loaded_callback) {
...
Database.prototype.add_song = function (song) {
...
function Song(data) {
  ...
  parts.push(this.data.album);
  parts.push(this.data.artist);
...
function DatabaseView(db) {
  ...
  this.$playall.on("click", function (e) {
    e.preventDefault();
    var filenames = [];
    _.each(self.songviews, function (sv) {
      if (sv.visible) {
...
DatabaseView.prototype.songSortFunction = function (song1, song2) {
  ...
  var s;
  s = sort_func(song1.data.album, song2.data.album);
  if (s !== 0) {
    return s;
  }
  s = sort_func(song1.data.discnum, song2.data.discnum);
  if (s !== 0) {
    return s;
  }
  ...
  s = sort_func(song1.data.name, song2.data.name);
  if (s !== 0) {
    return s;
  }
  return 0;
...
SongView.prototype.hide = function () {
  this.$el.hide();
  this.visible = false;
};
SongView.prototype.show = function () {
  this.$el.show();
  this.visible = true;
};
...
$(function () {
  ...
  function loaded_callback() {
    ...
    var dbview = window.dbview = new DatabaseView(db);
    $('#main').append(dbview.$el);
